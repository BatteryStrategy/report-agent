...
- missing_comparison_items: 재무/기술/시장/리스크 중 언급 없는 항목
...
def _check_cross_validation(
    ...
    """
    내부(RAG)·외부(Web) 교차 근거가 최소 1개 이상인지 확인한다.
    LGES 또는 CATL 중 하나라도 RAG + Web 양쪽 출처를 모두 사용했으면 True.
    """
    def both_sources(data: dict) -> bool:
        return (
            data.get("rag_doc_count", 0) > 0
            and data.get("web_result_count", 0) > 0
        )
    return both_sources(lges_data) or both_sources(catl_data)
...
def validation_node(state: GraphState) -> GraphState:
    ...
    # ── 4. 교차 근거 판정 ────────────────────────────────
    cross_validated = _check_cross_validation(lges_data, catl_data)
    cross_issue: list[str] = []
    if not cross_validated:
        cross_issue = [
            "교차 근거 미충족: LGES·CATL 중 RAG+Web 양쪽 출처를 동시에 사용한 항목 없음"
        ]

    # ── 5. 종합 판정 ─────────────────────────────────────
    all_issues = bias_issues + missing_items + source_issues + cross_issue
